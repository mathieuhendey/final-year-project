...
class CurrentAnalyses(object):
    ...
    def on_get(self, req: falcon.Request, resp: falcon.Response):
        if req.get_param(constants.TWEET_TOPIC_TABLE_KEY_NAME):
            if self.current_analyses_table.find_one(
                    analysis_topic_id=req.get_param_as_int(constants.TWEET_TOPIC_TABLE_KEY_NAME)):
            ...
            if self.current_analyses_table.find_one(
                    analysis_user_id=req.get_param_as_int(constants.TWEET_USER_TABLE_KEY_NAME)):
                resp.status = falcon.HTTP_OK
                resp.body = dumps({'currently_analysing': True})
            else:
                resp.status = falcon.HTTP_OK
            ...
            current_analyses = self.current_analyses_table.all()
